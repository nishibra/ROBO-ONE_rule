...
- (6)__V3.2_2025/12/23改訂(しゃがみ状態の規定改定,予選一部変更)__
...
- (c)移動、攻撃のときは、(a)に加えて、両脚の足首関節が同時に45度以下 ~~（逆関節の場合も同様）~~ __（前後方向いずれかが45度以下の場合）__ となるときしゃがみ状態とする。ただし大技および起き上がり動作はこの限りではない。
...
- (c) __予選競技開始後、図8.3に示すように直径50mmの丸いマーカー(色付きシール)から時計回りに歩行し、同色のマーカーを廻り中央のパイロンを確保に向かい歩行する。（移動ではなく、足を挙げての歩行動作とし、しゃがみによる歩行・移動も禁止とする。ただしその場旋回は可とする。転倒した場合、すぐに起き上がり歩行を続ける。歩行が全くできない場合あるいはレフリーが歩行と判断できなかった場合は失格とする。ロボットの規格5.(b)に従い歩行を審査する場合がある。)
最初にロボットがパイロン触れた時、レフリーのファイトの合図ですべてのロボットはパイロンを奪い合うバトルを開始する。__
- (d)~~予選競技開始後、レフリーの10カウント以降は~~ __パイロンを奪い合うバトルを開始後は自由に移動でき__ ダウン/スリップの判定もせず、しゃがみ攻撃や寝技も許可する。なお、ロボットが以下の状態になった場合、レフリーの判断で失格とする。
