...
- (a)	足裏(地面に接地する部分)のサイズを規定する。足裏の前後の長さは、__10cm以下__とする。足裏の左右の長さは脚の長さの__5cm以下__とする。
  ただし、足の長さは10cm以上とする。脚の長さとは、脚部の最上部にある、前後に動くより高い軸から足裏までの長さとし、脚を伸ばした状態で長さを計測する。 (図D-3参照)
図3
図4
...
#### 5.3.2　腕の規定 
- (a) 胴体から離れて動く部位（脚を除く腕、首など）の長さは胴体から離れて動く部位を支える軸の中心から20cm以下とする。長さの計測は攻撃を想定し前後に伸ばした状態で行う。（図D-7参照）なお公認ロボットの場合は公認ロボットの規格に準じる。
